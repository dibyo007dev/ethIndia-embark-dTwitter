...
class UpdateUser extends Component {
  ...
  _handleClick = async () => {
    ...
    const editAccount = DTwitter.methods.editAccount(
      web3.utils.keccak256(user.username),
      description,
      hash
    );
    ...
    try {
      ...
      const result = await editAccount.send({
        from: web3.eth.defaultAccount,
      ...
      if (
        result.status &&
        !Boolean(result.status.toString().replace("0x", ""))
      ) {
        return this.setState({
          isLoading: false,
          formState: "error",
          formUpdated: false,
          error:
            "Error executing transaction, transaction details: " +
    ...
    return (
      <Grid>
        <Row>
          <Col xs={12}>
            <PageHeader>
              Update {user.username} <small>{this.props.account}</small>
            </PageHeader>
          </Col>
        </Row>
        <Row>
          <Col xs={12}>
            ...
            >
              ...
              <FormGroup>
                {user.picture.length ? (
                  <Image src={user.picture} width="100" circle />
                ) : (
                  ""
                )}
              </FormGroup>
              ...
              <FormGroup validationState={formState}>
