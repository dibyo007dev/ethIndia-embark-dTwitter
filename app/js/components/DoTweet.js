...
class DoTweet extends Component {
   ...
   */
  _getValidationState() {
    return (this.state.tweet === "" && !this.state.tweetHasChanged) ||
      (this.state.tweet.length > 0 && this.state.tweet.length <= 140)
      ? null
      : "error";
  }
  //#endregion
    ...
    return (
      <form>
        ...
        <Button
          ...
          onClick={
            !isValid || Boolean(error) || !tweetHasChanged
              ? null
              : e => this._handleClick(e)
          }
        >
          {isLoading ? "Loading..." : "Post tweet"}
        </Button>
