...
class UserTweets extends Component {
  ...
  _getUserDetails = async username => {
    ...
    let user = await DTwitter.methods
      .users(web3.utils.keccak256(username))
      .call();

    // update picture url for ipfs
    user.picture =
      user.picture.length > 0
        ? EmbarkJS.Storage.getUrl(user.picture)
        : imgAvatar;
  ...
  render() {
    ...
    if (user === {}) {
      ...
      return (
        <Grid>
          <Row>
            <Col xs={12}>
              <PageHeader>
                {this.props.match.params.username} <small>doesn't exist!</small>
              </PageHeader>
            </Col>
          </Row>
        </Grid>
      ...
      const tweetList = this.state.tweets.map(function(tweet, index) {
        return (
          <ListGroupItem className="tweet" key={index} header={tweet.time}>
            {tweet.content}
          </ListGroupItem>
        );
      });
      return (
        <Grid>
          ...
          <Row>
            ...
            <Col xs={8}>
              <ListGroup className="tweets">{tweetList}</ListGroup>
            </Col>
          </Row>
        </Grid>
      );
