...
class CreateUser extends Component {
  ...
  _handleClick = async () => {
    ...
    try {
      ...
      const createAccount = DTwitter.methods.createAccount(
        username,
        description
      );
      ...
      const result = await createAccount.send({
        from: web3.eth.defaultAccount,
    ...
    if (input === "username") {
      ...
      if (value.length >= 5) {
        ...
        if (!this.state.isLoading) {
          ...
          DTwitter.methods
            .userExists(web3.utils.keccak256(value))
            .call()
            .then(exists => {
              // stop loading state
  ...
  render() {
    ...
    let isValid =
      validationState === "success" && !isLoading && !this.state.error;
    let feedback = isValid
      ? "Username is available"
      : this.state.error ||
        "Usernames must be 6 or more characters and cannot include @ or spaces.";
    ...
    return (
      <Grid>
        <Row>
          <Col xs={12}>
            <PageHeader>
              Create a user <small>for {this.props.account}</small>
            </PageHeader>
          </Col>
        </Row>
        <Row>
          <Col xs={12}>
            <form onSubmit={!isValid ? null : e => this._handleClick(e)}>
              <FieldGroup
                ...
                onKeyPress={e =>
                  e.key === "@" || e.key === " " ? e.preventDefault() : true
                }
